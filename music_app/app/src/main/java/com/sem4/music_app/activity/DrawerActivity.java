        ...
        if (Constant.arrayList_play.size() > 0) {
            ...
            if (Constant.isOnline || Constant.isDownloaded) {
                ...
                if (checkPer()) {
                    ...
                    share.putExtra(Intent.EXTRA_STREAM, Uri.parse(Constant.arrayList_play.get(viewpager.getCurrentItem()).getMediaUrl()));
...
//        dialog_rate = new Dialog(DrawerActivity.this);
...
//            seekUpdation();
    ...
    private class ImagePagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(@NonNull ViewGroup container, int position) {
            ...
            if (Constant.isOnline) {
                Picasso.get()
                        .load(Constant.arrayList_play.get(position).getImage())
                        .resize(300, 300)
                ...
                Uri uri;
                if (Constant.isDownloaded) {
                    uri = Uri.fromFile(new File(Constant.arrayList_play.get(position).getImage()));
                } else {
                    uri = methods.getAlbumArtUri(Integer.parseInt(Constant.arrayList_play.get(position).getImage()));
                }
                Picasso.get()
                        .load(uri)
                        .placeholder(R.drawable.placeholder_song)
    ...
    private void openOptionPopUp() {
        PopupMenu popup = new PopupMenu(DrawerActivity.this, iv_max_option);
