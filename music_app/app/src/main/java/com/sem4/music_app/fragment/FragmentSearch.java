        ...
        methods = new Methods(getActivity(), new OnClickListener() {
            ...
            public void onClick(int position, String type) {
                ...
                if(type.equals(getString(R.string.artist))) {
                    ...
                    intent.putExtra("id", arrayList_artist.get(position).getId().toString());
                    ...
                    intent.putExtra("id", arrayList_album.get(position).getId().toString());
...
//        });
    ...
    private void loadSongs() {
        if (methods.isNetworkAvailable()) {
            ...
            apiManager.searchHomePage(Constant.search_item)
                    .enqueue(new Callback<BaseResponse<ItemSearch>>() {
                        @Override
                        public void onResponse(Call<BaseResponse<ItemSearch>> call, Response<BaseResponse<ItemSearch>> response) {
                            errr_msg = getString(R.string.err_no_data_found);
                            arrayList_songs.addAll(response.body().getContent().getSearchSong());
                            arrayList_artist.addAll(response.body().getContent().getSearchArtist());
                            arrayList_album.addAll(response.body().getContent().getSearchAlbum());
                        ...
                        @Override
                        public void onFailure(Call<BaseResponse<ItemSearch>> call, Throwable t) {
